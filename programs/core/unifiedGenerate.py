...
# ─────────────────────────────────────────────────────────────────────────────
KAISER_CONSTANT = 3.336  # Empirical constant for beta=10 Kaiser window
...
def _compute_case_params(f1: float, f2: float) -> dict:
    ...
    fs_eff = FS_NATIVE / D
    return dict(f1=f1, f2=f2, D=D, fs_eff=fs_eff)
...
def _welch_1024(x_in: np.ndarray, fs_eff: float, Nz: int) -> np.ndarray:
    ...
    # Step 1
    s  = np.real(x_in * np.conj(x_in)).astype(np.float64)
    s -= s.mean()
...
def extract_welch_frame(
    ...
    if no_decimation:
        ...
        x_in   = _decimate(noisy_signal, FS_NATIVE, cpd['D'], use_gpu=use_gpu)
        fs_eff = cpd['fs_eff']
...
def generate_unified_case_dataset(
    ...
    if no_decimation:
        ...
        ND         = int(N_RAW / D)
        Nz         = _compute_nz(ND)
        Ns_no_ovlp = (ND + Nz) // CREPE_FRAME_LENGTH
        mode_label = f"decimated (D={D})"
        mode_tag   = f"D{D}"
