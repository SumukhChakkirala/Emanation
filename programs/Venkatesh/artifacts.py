...
def phaseOffset(samples, seed=None, rng=None):
    """Apply a random phase offset using a NumPy Generator or seed.

    Args:
        samples: Complex IQ samples.
        seed: Optional integer seed for backward compatibility.
        rng: Optional NumPy Generator.
    """
    if rng is None:
        rng = np.random.default_rng(int(seed) if seed is not None else None)
    phaseOffsetVal = rng.uniform(0, 2 * np.pi)
