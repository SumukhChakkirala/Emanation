...
class TFSameConv2d(nn.Module):
    """
    Conv2d layer that replicates TensorFlow's 'SAME' padding behavior
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride):
        ...
        # Conv2d with no padding - we'll pad manually in forward
        self.conv = nn.Conv2d(
    ...
    def forward(self, x):
        ...
        # Apply stride by slicing (matches TF's window alignment exactly)
        if self.stride[0] > 1 or self.stride[1] > 1:
            x = x[:, :, ::self.stride[0], ::self.stride[1]]
    ...
    def __init__(self, model_capacity='full'):
        ...
        for i, (f, w, s) in enumerate(zip(filters, widths, strides)):
            block = nn.Sequential(
                ...
                nn.ReLU(),
                nn.BatchNorm2d(f, eps=0.001, momentum=0.01),  # TF defaults
...
def load_tf_weights(model, tf_model_path):
    ...
    print("⚠️  Weight conversion not yet implemented!")
    print("You'll need to:")
    print("1. Load TF weights from .h5 file")
    print("2. Map each TF layer to corresponding PyTorch layer")
    print("3. Handle shape conversions (TF uses NHWC, PyTorch uses NCHW)")
    print("4. Copy weights into model.state_dict()")
    
    raise NotImplementedError("TensorFlow weight loading not yet implemented")
...
if __name__ == "__main__":
    ...
    dataset_path = r'C:\Users\User2\Documents\Emanation\crepe_dataset.pkl'
    ...
    print("\n⚠️  Note: These results are from random weights.")
    print("   For accurate pitch estimation, load trained weights:")
    print("   model.load_state_dict(torch.load('crepe_weights.pth'))")
    
    print("\n📐 CREPE Architecture Summary:")
    print("   • Input: 1024 samples (64ms @ 16kHz)")
    print("   • 6 Conv blocks: Conv2D → ReLU → BatchNorm → MaxPool → Dropout")
