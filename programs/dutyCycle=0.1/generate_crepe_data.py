...
if __name__ == "__main__":
    OUTPUT_DIR = './IQData/'
    # Update to new folder under dutyCycle=0.1
    OUTPUT_DIR = os.path.join(os.path.dirname(__file__), 'IQData')
    OUTPUT_FILE = 'iq_dict_crepe_dirac_comb_SNRneg20_20.pkl'
    ...
    iq_dict = generate_crepe_dataset_dense(
        ...
        snr_list=list(range(-20, 21)),  # SNR range: -20 to 20 dB (41 levels)
        samples_per_bin_snr=25,  # 41 SNRs × 25 augments × ~355 bins ≈ 363,875 samples
        duty_cycle=duty_cycle,
        seed=42
    )
    
    # Total with fmin=100: 258 bins × 6 SNRs × 250 augmentations = ~387,000 samples
    visualize_dataset(iq_dict, n_samples=21)
    # Optionally, you can save models in the new models folder as needed
